				...
				<div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
					...
					{sauceInfo && (
						<div>
							...
							<div style={{ color: '#fff' }}>
								{sauceInfo.fileTypeDescription} (DataType: {sauce.dataType}, FileType:{' '}
								{sauce.fileType})
							</div>
						</div>
					)}
					...
					<div>
						...
						<div style={{ color: '#fff', fontSize: '12px', fontFamily: 'monospace' }}>
							TInfo1: {sauce.tInfo1} | TInfo2: {sauce.tInfo2} | TInfo3: {sauce.tInfo3} | TInfo4:{' '}
							{sauce.tInfo4}
						</div>
					</div>
					...
					<div>
						...
						<div style={{ color: '#fff', display: 'flex', flexDirection: 'column', gap: '4px' }}>
							...
							{!sauceInfo?.iceColors && !sauceInfo?.letterSpacing && (
					...
					>
						<div
							style={{ color: '#888', marginBottom: '12px', fontSize: '12px', fontWeight: 'bold' }}
						>
							Technical Details
						</div>
						<div
						...
						>
							...
							<div>
								tFlags: 0x{sauce.tFlags.toString(16).toUpperCase().padStart(2, '0')} ({sauce.tFlags}
								)
							</div>
					...
					{sauce.comments > 0 && sauce.commentLines.length > 0 && (
						<div>
							<div style={{ color: '#888', marginBottom: '8px' }}>Comments ({sauce.comments}):</div>
							<div
								style={{
									color: '#fff',
							...
							>
								{sauce.commentLines.map((comment, idx) => (
									<div
										key={idx}
										style={{ marginBottom: idx < sauce.commentLines.length - 1 ? '8px' : '0' }}
									>
										{comment}
									</div>
								))}
